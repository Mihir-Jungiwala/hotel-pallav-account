...
@include('reports.partials._totals', ['cards' => ['Total spent' => $money($totals['total']), 'Hotel Pallav' => $money($totals['hotel']), 'Pallav Food' => $money($totals['food']), 'Heads used' => $totals['heads']]])

<div class="card reveal">
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0" data-paginate="25" data-pager="#reportPager">
            <thead><tr>
                <th>Expense head</th><th class="text-end">Entries</th>
            ...
            @forelse($rows as $row)
                <tr data-row="{{ $row['head'] }}">
                    <td class="fw-semibold">{{ $row['head'] }}</td>
