            ...
            @forelse($entries as $row)
                <tr data-row="{{ optional($row->employee)->name }} {{ $row->type }} {{ $row->remarks }}">
                    ...
                    <td class="text-end text-nowrap">
                        <a class="btn btn-sm btn-outline-p" href="{{ route('payroll.bonus-incentive.view', $row) }}" target="_blank"><i class="bi bi-eye"></i></a>
                        <button class="btn btn-sm btn-outline-p" data-bs-toggle="modal" data-bs-target="#editBonus{{ $row->id }}"><i class="bi bi-pencil"></i></button>
