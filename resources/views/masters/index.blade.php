        ...
        @if($current)
            <div class="card reveal">
                <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <div class="d-flex gap-2">
                        <button class="btn btn-sm btn-outline-p" data-bs-toggle="modal" data-bs-target="#editSet"><i class="bi bi-sliders"></i> List settings</button>
                        <button class="btn btn-sm btn-p" data-bs-toggle="modal" data-bs-target="#newItem"><i class="bi bi-plus-lg"></i> Add option</button>
                    </div>
                </div>
                            ...
                            <div class="modal fade" id="editItem{{ $item->id }}" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
                                <form method="POST" action="{{ route('masters.items.update', $item) }}">@csrf @method('PUT')
                                    ...
                                    <div class="modal-body">
                                        @include('masters._item-fields', ['item' => $item])
                                    </div>
                            ...
                            <tr>
                                <td colspan="5">
                                    <div class="empty-state">
                                        ...
                                        <div class="es-text">Add the first option and it appears on every form that uses {{ $current->name }}.</div>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div class="modal fade" id="newItem" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
                <form method="POST" action="{{ route('masters.items.store', $current) }}">@csrf
                    <div class="modal-header">
                        <h5 class="modal-title">Add to {{ $current->name }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">@include('masters._item-fields', ['item' => null])</div>
                    <div class="modal-footer"><button class="btn btn-p">Add Option</button></div>
                </form>
            </div></div></div>
